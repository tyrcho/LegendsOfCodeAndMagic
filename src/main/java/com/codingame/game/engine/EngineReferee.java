...
public class EngineReferee {
  ...
  public void refereeInit(MultiplayerGameManager<Player> gameManager) {
    ...
    if (Constants.VERBOSE_LEVEL > 1)
      System.out.println("   Draw Phase Prepared. " + draft.allowedCards.size() + " cards allowed. ");
    if (Constants.VERBOSE_LEVEL > 1)
      System.out.println("   " + draft.draftingCards.size() + " cards selected to the draft.");
  ...
  private void DraftTurn(MultiplayerGameManager<Player> gameManager, Runnable render) {
    ...
    if (gameTurn == 0) {
      ...
      gameManager.setTurnMaxTime(Constants.TIMELIMIT_FIRSTDRAFTTURN);
    } else gameManager.setTurnMaxTime(Constants.TIMELIMIT_DRAFTTURN);
  ...
  private boolean GameTurn(MultiplayerGameManager<Player> gameManager, Runnable render) {
    ...
    } else // it's time to actually call a player
    {
      if (Constants.VERBOSE_LEVEL > 2)
        System.out.print("      Game turn " + (gameTurn - Constants.CARDS_IN_DECK) + ", player " + gamePlayer);

      if (gameTurn <= Constants.CARDS_IN_DECK + 1) gameManager.setTurnMaxTime(Constants.TIMELIMIT_FIRSTGAMETURN);
      else gameManager.setTurnMaxTime(Constants.TIMELIMIT_GAMETURN);
  ...
  private boolean CheckAndHandleEndgame(MultiplayerGameManager<Player> gameManager, GameState state) {
    ...
    if (Constants.VERBOSE_LEVEL > 1)
      System.out.println("   Game finished in turn " + (gameTurn - Constants.CARDS_IN_DECK) + ".");
